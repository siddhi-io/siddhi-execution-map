...
public class PutIfAbsentFunctionExtensionTestCase {
    private static final Logger log = LogManager.getLogger(PutIfAbsentFunctionExtensionTestCase.class);
    ...
    public void testPutFunctionExtension() throws InterruptedException {
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                ...
                "select symbol,price,tmpMap, map:putIfAbsent(tmpMap,symbol,price)" +
                " as newmap"
    ...
    public void testPutFunctionExtension2() throws InterruptedException {
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                ...
                + "map:putIfAbsent(map:put(map:create(),'IBM',50),symbol,price) as newmap"
                + " insert into outputStream;"
        );
        ...
        siddhiAppRuntime.addCallback("outputStream", new StreamCallback() {
            ...
            public void receive(Event[] events) {
                ...
                for (Event event : events) {
                    ...
                    HashMap map = (HashMap) event.getData(3);
                    if (count.get() == 1) {
                        AssertJUnit.assertEquals(50, map.get("IBM"));
                        eventArrived = true;
                    }
                    if (count.get() == 2) {
                        AssertJUnit.assertEquals(200, map.get("WSO2"));
    ...
    public void testPutFunctionExtension1() {
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                + "map:create() as tmpMap insert into tmpStream;"
                + "@info(name = 'query2') from tmpStream  "
                + "select symbol,price,tmpMap, map:putIfAbsent(tmpMap,symbol) as newmap"
                + " insert into outputStream;"
        );
